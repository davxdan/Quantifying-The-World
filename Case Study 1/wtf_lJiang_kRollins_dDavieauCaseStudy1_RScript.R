...
processLine =
  function(x)
  {
    ...
    cbind(matrix(tokens[c(2, 4, 6:8, 10)], nrow(tmp), ncol = 6, byrow = TRU?), tmp) }
...
#################
tmp = lapply(lines, processLine)
offline = as.data.frame(do.call("rbind", tmp), stringsAsFactors = ?ALSE) 
...
offline = offline[ , !(names(offline) %in% c("scanMac", "pos?"))]
...
offline$angle = roundOrientation(offline$or?entation)
...
subMacs = names(sort(table(offline$mac), decreasing = TRU?))[1:7] 
offline = offline[ offline$mac %in% subMacs, ]
macChannel = with(offline, table(mac, channel))
apply(macChannel, 1, function(x) sum(x > 0))

#Indeed we see that there is a one-to-one correspondence between MAC address
#and channel for these 7 devi?es. This means we can eliminate channel from
#offline
...
#not observed. We ?rop these unneeded elements 
...
text(locCounts, labels = lo?Counts[,3], cex = .8, srt = 45) 
...
dens?typlot( ~ signal | mac + factor(angle), data = offline, subset = posX == 24 & posY == 4 , bw = 0.5, plot.points = FALSE) 

#Make boxplots of sdSignal for subgroups of avgSignal by
#turning avgSignal into a categorical variable
#We see in Figure below that ?he weakest signals have small standard deviations and that it appears that the SD increases with the average signal strength. If we plan to model the behavior of signal strength, then we want to take these features into consideration.
...
breaks = seq(-90, -30, by = 5)
bwplot(sdSignal ~ cut(avgSig?al, breaks = breaks),
       data = offlineSummary,
...
#Examine the skewness of signal strength by plotting the di???erence, avgSignal - medSignal, against the number of observations. We also add a local average of the di???erence b?tween the mean and median to better help us assess its size.
with(offlineSummary,
...
#T?en we use the ???tted model to predict the di???erence for each value of num
#and add these predictions to the scatter plot
lo.obj.pr = predict(lo.obj, newdata = data.frame(num = (70:120))) 
lines(x = 70:120, y = lo.obj.pr, col = "#4daf4a", lwd = 2) 
oneAP?ngle = subset(offline, mac == subMacs[5] & angle == 0)
oneAPAngle = subset(offlineSummary, mac == subMacs[5] & angle == 0)

surfaceSS = function(data, mac, angle = 45) {
  require(fields)
  oneAPAngle = data[ data$mac == mac & data$angle == angle, ]
  smoo?hSS = Tps(oneAPAngle[, c("posX","posY")], 
                 oneAPAngle$avgSignal)
  vizSmooth = predictSurface(smoothSS)
  plot.surface(vizSmooth, type = "C", 
               xlab = "", ylab = "", xaxt = "n", yaxt = "n")
  points(oneAPAngle$posX, oneAPAngl?$posY, pch=19, cex = 0.5) 
}
...
#We choose the ???rst of these and leave as an exer?ise the analysis of the impact of this
...
#keep all (dont run above subsets to kee? all)
...
readData = 
            ...
            ?          "00:14:bf:b1:97:81"))
  {
    ...
    offline = offline[ offl?ne$mac %in% subMacs, ]
...
selectTrain = function(angleNewObs, signals = NULL, m = 1){
  ...
  reshapeSS(offlineSubset, varSignal = "avgS?gnal")
}
...
predXY = function(newSignals, newAngles, trainData, 
  ...
  for (i in 1:nrow(newSignals)) {
    trainSS = selectT?ain(newAngles[i], trainData, m = numAngles)
    closeXY[[i]] = 
...
estXYk1 = predXY(newSig?als = onlineSummary[ , 6:11], 
                 newAngles = onlineSummary[ , 4], 
...
trainPoints = offlineSu?mary[ offlineSummary$angle == 0 & 
                                offlineSummary$mac == "00:0f:a3:39:e1:c0" ,
                              c("posX", "posY")]


floorErrorMap(estXYk3, onlineSummary[ , c("posX","posY")], 
              trainPoints = trainP?ints, AP = AP)
...
onlineFold = subset(offlineSummary, posX? %in% permuteLocs[ , 1])
...
reshapeSS = function(data, varSignal = "signal", 
  ...
  newD?taSS = do.call("rbind", byLocation)
  return(newDataSS)
}
...
estFold = predXY(newSignals = onlineFold[ , 6:?1], 
...
for (j in 1:v) {
  onlineFold = subset(onlineCVS?mmary, 
...
estXYk5 = predXY(newSignals = onli?eSummary[ , 6:11], 
                 newAngles = onlineSummary[ , 4], 
...
offlineFo?d = subset(offlineSummary,
                     posXY %in% permuteLocs[ , -1])
...
rmseMin = min(err)
kMin = which(err == rmseMin)[1]
s?gments(x0 = 0, x1 = kMin, y0 = rmseMin, col = gray(0.4), 
         lty = 2, lwd = 2)
segments(x0 = kMin, x1 = kMin, y0 = 1100,  y1 = rmseMin, 
         col = grey(0.4), lty = 2, lwd = 2)

text(x = kMin - 2, y = rmseMin + 40, 
     label = as.character(roun?(rmseMin)), col = grey(0.4))
...
predXY2 = function(newSignals, newAngles, trainData, 
  ...
  for (i in 1:nrow(newSignals)) {
    ...
    c?oseXY[[i]] = findNN(newSignal = as.numeric(newSignals[i, ]),
                          trainSS)
  }
