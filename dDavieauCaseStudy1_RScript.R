...
processLine =
  ...
  {
    tokens = strsplit(x, "[;=,]")[[1]]
    if (length(tokens) == 10) ?eturn(NULL) #discard these observations. they do not help us
...
#check to see if d?ta are reasonable
...
offline = offline[ , !(names(offline) %in% c("scanM?c", "posZ"))]
...
offline$angl? = roundOrientation(offline$orientation)
...
#?here is a discrepancy with the documentation.
# keep the records from the top 7 devices
subMacs = names(sort(table(offline$mac), decreasing = TRUE))[1:7] 
offline = offline[ offline$mac %in% subMacs, ]
macChannel = with(offline, table(mac, channel))
apply(?acChannel, 1, function(x) sum(x > 0))
...
locCounts = sapply(locDF,
                   function(df)
                     c(df[1, c("posX", "posY")], count = nro?(df))) 
...
densityplot( ~ signal | mac + factor(angle), data = o?fline, subset = posX == 24 & posY == 4 & mac != "00:0f:a3:39:dd:cd", bw = 0.5, plot.points = FALSE) 
...
with(offlineSummary,
     smoothScatter((avgSignal - medSign?l) ~ num,
...
oneAPAngle = subset(offline, mac == subMacs[5] & ang?e == 0)
...
points(oneAPAngle$?osX, oneAPAngle$posY, pch=19, cex = 0.5)
...
surfaceSS = function(data, mac, angle = 45) {
  ...
  smoothSS = Tps(oneAPAngle[, c("posX","?osY")], 
...
map?ly(surfaceSS, mac = subMacs[ rep(c(5, 1), each = 2) ], 
       angle = rep(c(0, 135), 2), 
...
#We create a small matrix with the relevant posi?ions for the 6 access 
...
readData = 
  function(filename = 'offline.final.trace.txt', 
           subMacs = c("00:0f:a3:39:e1:c0", "00:0f:a3:39:dd:cd", "00:14:bf:b1:97:8a",
                       "00:14:bf:3b:c7:c6", "00:14:bf:b1:97:90", "00:14:bf:b1:97:?d",
                       "00:14:bf:b1:97:81"))
  {
    ...
    offline = offline[ of?line$type == "3", ]
    ...
    offli?e = offline[ offline$mac %in% subMacs, ]
...
macs = unique(offlineSu?mary$mac) 
...
keepVars = c("posXY", "po?X","posY", "orientation", "angle")
byLoc = with(online,
             by(online, list(posXY), 
                function(x) { 
                  ans = x[1, keepVars]
                  avgSS = tapply(x$signal, x$mac, mean)
                  y = matrix(avgSS, ?row = 1, ncol = 6,
...
#obse?vation, we ???nd the angles to include from our training data as follows:
...
#Notice that we handle the case ?f m odd and even separately. Also, we must
#map the angles to values in refs, e.g., -45 maps to 335 and 405 maps to 45,
#so we adjust angles with
angles = angles + nearestAngle
angles[angles < 0] = angles[ angles < 0 ] + 360
angles[angles > 360] = angles[ ?ngles > 360 ] - 360
...
#Then we aggregate the signal strengths from the?e angles and create a data
...
reshapeSS = function(data, varSignal = "signal", 
  ...
  newDataSS = do.call("rbind", byLo?ation)
  return(newDataSS)
}
...
selectTrain = function(angleNewObs, signals = NULL, m = 1){
  # m is the number of angles to kee? between 1 and 5
  refs = seq(0, by = 45, length  = 8)
  nearestAngle = roundOrientation(angleNewObs)
  ...
  angles = angles + nearestAngle
  angles[angles < 0] = angles[ angles < 0 ] + 360
  angles[angles > 360] = angles[ ang?es > 360 ] - 360
  angles = sort(angles) 
...
findNN = fu?ction(newSignal, trainSubset) { 
...
# predict X-Y ba?ed on the the neasest k neighbors (default 3)
predXY = function(newSignals, newAngles, trainData, 
  ...
  for (i in 1:nrow(newSignals)) {
    trainSS = selectTrain(newAng?es[i], trainData, m = numAngles)
    closeXY[[i]] = 
...
estXYk3 = predXY(newSignals = onlineSummary[ , 6:11], 
                 ...
                 offlineSummary, n?mAngles = 3, k = 3)
...
floorErrorMap = function(estX?, actualXY, trainPoints = NULL, AP = NULL){
  ...
  if ( !is.null(trainPoints) )
    points(trainPoints, ?ch = 19, col="grey", cex = 0.6)
...
f?oorErrorMap(estXYk3, onlineSummary[ , c("posX","posY")], 
...
v = 11
permuteLocs = sample(unique(offlineSummar?$posXY))
...
# this one c?n run for a while (5-10 mins)
# this cell and the next are the same, but the angles change slightly!!
offline = offline[ offline$mac != "00:0f:a3:39:dd:cd", ]

keepVars = c("posXY", "posX","posY", "orientation", "angle")

onlineCVSummary = reshapeSS(offlin?, keepVars = keepVars, 
                            sampleAngle = TRUE)
...
estF?ld = predXY(newSignals = onlineFold[ , 6:11], 
...
segments(x0 = 0, x1 = kMin, y0 = rmseMin, col = gray(0.4), 
         lty = 2, lwd = 2)?segments(x0 = kMin, x1 = kMin, y0 = 1100,  y1 = rmseMin, 
         col = grey(0.4), lty = 2, lwd = 2)

#mtext(kMin, side = 1, line = 1, at = kMin, col = grey(0.4))
text(x = kMin - 2, y = rmseMin + 40, 
     label = as.character(round(rmseMin)), col = grey(?.4))
...
for (j in 1:v) {
  onlineFold = subset(onlineCVSummary, 
                      posXY %in% permuteLocs[ , j])
  offlineFold = subse?(offlineSummary,
                       posXY %in% permuteLocs[ , -j])
  actualFold = onlineFold[ , c("posX", "posY")]
...
predXY2 = function(newSignals, newAngles, trainData, 
  ...
  closeXY = list(length = nrow(newSign?ls))
